...
// with the default "/twirp" prefix and default CamelCase service and method names.
// More info: https://twitchtv.github.io/twirp/docs/routing.html
const AdminPathPrefix = "/twirp/xeiaso.net.internal.Admin/"
...
func (s *adminServer) ServeHTTP(resp http.ResponseWriter, req *http.Request) {
	...
	// Verify path format: [<prefix>]/<package>.<Service>/<Method>
	prefix, pkgService, method := parseTwirpPath(req.URL.Path)
	if pkgService != "xeiaso.net.internal.Admin" {
		msg := fmt.Sprintf("no handler for path %q", req.URL.Path)
		s.writeError(ctx, resp, badRouteError(msg, req.Method, req.URL.Path))
		return
	}
...
func (s *adminServer) serveRebuild(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	switch strings.TrimSpace(strings.ToLower(header[:i])) {
	case "application/json":
		s.serveRebuildJSON(ctx, resp, req)
	case "application/protobuf":
		s.serveRebuildProtobuf(ctx, resp, req)
	default:
		msg := fmt.Sprintf("unexpected Content-Type: %q", req.Header.Get("Content-Type"))
		twerr := badRouteError(msg, req.Method, req.URL.Path)
		s.writeError(ctx, resp, twerr)
...
func (s *adminServer) serveRebuildJSON(ctx context.Context, resp http.ResponseWriter, req *http.Request) {
	...
	if s.interceptor != nil {
		handler = func(ctx context.Context, req *google_protobuf.Empty) (*xeiaso_net.BuildInfo, error) {
			resp, err := s.interceptor(
				func(ctx context.Context, req interface{}) (interface{}, error) {
					typedReq, ok := req.(*google_protobuf.Empty)
					if !ok {
	...
	// Call service method
	var respContent *xeiaso_net.BuildInfo
	func() {
		defer ensurePanicResponses(ctx, resp, s.hooks)
		respContent, err = handler(ctx, reqContent)
	}()
	...
	if respContent == nil {
		s.writeError(ctx, resp, twirp.InternalError("received a nil *xeiaso_net.BuildInfo and nil error while calling Rebuild. nil responses are not supported"))
		return
	}
...
func (s *adminServer) PathPrefix() string {
	return baseServicePath(s.pathPrefix, "xeiaso.net.internal", "Admin")
}
...
var twirpFileDescriptor0 = []byte{
	...
	,
}
